...
const showToastOnce = (type, message, duration = 5000) => {
  const key = `${type}:${message}`;
  const now = Date.now();
  const lastShown = toastStore.get(key);
  
  // Only show if not shown before or if > duration has passed
  if (!lastShown || (now - lastShown) > duration) {
    toast[type](message);
    toastStore.set(key, now);
  }
};
...
function normalizeBatchDto(dto) {
  ...
  const programmeName =
    ...
    "";
    const paperCode =
    get(["PaperCode", "paperCode", "papercode"]) ?? "";

  const paperName =
    get(["PaperName", "paperName", "papername"]) ?? "";
...
const BatchTab = () => {
  ...
  const fetchBoards = async () => {
    try {
      ...
      showToastOnce("error", "❌ Failed to load courses");
    }
  };
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE_URL}/Examination/GetAssignSubjectsByProgrammeID?courseId=${courseId}`,
      {
        headers: {
  ...
  const handleDelete = async (row) => {
    ...
    try {
      ...
      showToastOnce("error", `❌ Delete failed: ${err.message}`);
    }
  };
...
const handleSave = async () => {
 const {
  ...
  if (!trimmedBatchName || !startDate || !endDate || !programmeId) {
    showToastOnce("error", "Please fill all required fields: Batch, Start Date, End Date, and Course.");
    return;
  }
  if (new Date(startDate) > new Date(endDate)) {
    showToastOnce("error", "Start Date cannot be after End Date");
    return;
  }
  ...
  if (programmeId === "ALL") {
    ...
    if (!boards || boards.length === 0) {
      showToastOnce("error", "No courses available to save for ALL.");
      return;
    }
    ...
    const dto = {
      ...
      ExaminationId: Number(subjectId), // NEW
      Fee: 0,                           // IMPORTANT
  ...
  try {
    ...
    showToastOnce("error", `❌ Save failed: ${err.message}`);
  } finally {
    setSaving(false);
  }
};
...
<div className="col-12 col-md-6 col-lg-3">
  <Form.Group>
    ...
    <Form.Control
      ...
      disabled={!form.programmeId || form.programmeId === "ALL"}
    >
                    ...
                    return (
                      <tr key={b.bid || idx}>
                        ...
                        <td data-label="Programme" className="text-start">
                         ...
                         <td data-label="Course" className="text-start">
                          {b.paperName || <em>(no course)</em>}
                        </td>
